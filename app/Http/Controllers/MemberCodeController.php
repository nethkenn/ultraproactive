...
class MemberCodeController extends MemberController
{
	...
	public function addslotnoinsert($data)
	{
		...
		if(strtolower(Request::input("slot_position")) == 'left' || strtolower(Request::input("slot_position")) == 'right')
		{
				...
				elseif(Request::input("placement") == 0)
				{
					$return["message"] = "This position is not allowed.";
				}
				elseif($ifused)
				{
					$return["message"] = "This code is already used";
	...
	public function getslotbyid($id)
	{
		...
		$data['count2'] = Tbl_product_code::where("account_id", Customer::id())->where('tbl_product_code.used',0)->voucher()->product()->orderBy("product_pin", "desc")->where('tbl_product_code.archived',0)->unused()->count();
	...
	public function add_code($x)
	{
			...
			$total_wallet = Tbl_wallet_logs::where("slot_id", $rcheck->slot_id)->wallet()->sum('wallet_amount');	
			...
			if($checking == true && $checking5 == true)
			{
				if($rcheck && $check)
				{
						$total = $total_wallet - $check->membership_price;
						...
						if($total >= 0)
						{
							if (!$validator->fails())
							{
								...
								{ 
									...
									Log::slot(Session::get('currentslot'),'Bought a membership code ('.$check->membership_name.') ',$negative_amount,"Bought Membership Codes",Session::get('currentslot'));  
